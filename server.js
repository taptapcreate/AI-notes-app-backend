...
const lengthGuides = {
    brief: {
        instruction: 'Provide a concise but informative summary. Include 5-7 bullet points covering the core concepts and key highlights. Keep it short but ensure the main context and important details are clear.',
        maxTokens: 1200
    },
    standard: {
        instruction: 'Provide a comprehensive and well-rounded summary. Include 10-15 bullet points covering the main themes with sufficient explanations and important details. Provide a good level of depth without being overly exhaustive.',
        maxTokens: 2500
    },
    detailed: {
    ...
    try {
        ...
        if (config.dailyFreeCreditsEnabled) {
            ...
            if (remoteVersion > currentVersion) {
                ...
                user.totalFreeCredits = config.welcomePackCredits || 12;
...
app.post('/api/credits/sync-welcome-pack', async (req, res) => {
    try {
        ...
        if (version > currentVersion) {
            user.totalFreeCredits = credits;
            user.welcomePackVersion = version;
            await user.save();

            return res.json({
                success: true,
                newBalance: user.totalFreeCredits,
...
app.post('/api/credits/use', async (req, res) => {
    try {
        ...
        const isDaily = config.dailyFreeCreditsEnabled ?? true;
        const currentFreeFieldName = isDaily ? 'freeCreditsRemaining' : 'totalFreeCredits';
